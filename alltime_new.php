...
if (isset($_SESSION['logged_in_levy'])) {
  ...
  $query = $pdo->prepare("SELECT songid, COUNT(rating), AVG(rating) FROM ratings GROUP BY songid HAVING COUNT(rating) > 2 ORDER BY 3 DESC LIMIT 1;");
  $query->bindValue(1, $wantedtime);

  $query->execute();
  $bangerid = $query->fetch();

  $query = $pdo->prepare("SELECT songid, COUNT(rating), AVG(rating) FROM ratings GROUP BY songid HAVING COUNT(rating) > 2 ORDER BY 3 ASC LIMIT 1;");
  $query->bindValue(1, $wantedtime);
  ...
  $playlistlinkshit = "https://www.youtube.com/watch_videos?video_ids=";
...
<html>
  ...
  <body>
    ...
    <div class="Halfpic">
      ...
      <div class="Halftext">
        ...
        <div>
          ...
          <hr width="80%" color="#FFFFFF" size="2" align="left" margin-bottom="-5px">
            <div width="80%">
               <h5 class="intro" style="color: #00cc99;">WHILE MARTIN WAS AROUND</h5>
              <!--
              <p class="intro">This was how the board looked when Martin was still around!</p>
              -->
              <p class="intro">While the Council was in session</p>
      ...
      <p style="text-align:center;"><br>&#9201; AFTER MARTIN PASSED OUT &#9201;<br><br></p>
      </div>
      </a>
      <p></p>
        ...
        foreach ($allusers as $user) { ?>
          ...
          /* THE BEST FROM THE USER*/

          $query = $pdo->prepare("SELECT songby, songid, AVG(rating), COUNT(rating) FROM ratings WHERE songby= ? GROUP BY songid HAVING COUNT(rating) > 2 ORDER BY 3 DESC LIMIT 1;");
          ...
          /* THE WORST FROM THE USER */

          $query = $pdo->prepare("SELECT songby, songid, AVG(rating), COUNT(rating) FROM ratings WHERE songby= ? GROUP BY songid HAVING COUNT(rating) > 2 ORDER BY 3 ASC LIMIT 1;");
          $query->bindValue(1, $user['name']);
        ...
        <div style="width:80%">
          <div id="goodsongstop"></div>
          <ol>
        <?php
        $top = new Raati;
        $top10 = $top->get_top10_afterhour($wantedtime);
      ...
      <?php } else {
        foreach ($top10 as $top) { ?>
              ...
              ?>
                          ...
                          <?php
                            parse_str( parse_url( $top10song['link'], PHP_URL_QUERY ), $my_array_of_vars);
      ...
      <div id="goodsongs">
        ...
        <p>
           <img src="playlist.png" height="10%"> Create YouTube playlist from this GOLD
        </p>
      ...
      <script>
      jQuery("#goodsongs").detach().appendTo('#goodsongstop')
      ...
      <div style="width:80%">
        <div id="shitsongstop"></div>
        <ol>
      <?php
      $bottom = new Raati;
      $bottom10 = $bottom->get_bottom10_afterhour($wantedtime);
    ...
    <?php } else {
      foreach ($bottom10 as $bottom) { ?>
            ...
            <?php
              parse_str( parse_url( $bottom10song['link'], PHP_URL_QUERY ), $my_array_of_vars);

              $youtubelink = $my_array_of_vars['v'];
              $youtubelink = trim($youtubelink);

              if(!empty($youtubelink)){
                $playlistlinkshit .= $youtubelink;
                $playlistlinkshit .= ",";
                  }
             ?>
  ...
  <div id="shitsongs">
    <a href="<?php echo $playlistlinkshit; ?>" target="_blank">
    <p>
       <img src="playlist.png" height="10%"> Create YouTube playlist from this shit
    </p>
  ...
  <script>
  jQuery("#shitsongs").detach().appendTo('#shitsongstop')
