...
if (isset($_SESSION['logged_in_levy'])) {
  ...
  $query = $pdo->prepare("SELECT songid, AVG(rating), COUNT(rating) FROM ratings WHERE thetime >= ? GROUP BY songid HAVING COUNT(rating) > 2 ORDER BY 2 DESC LIMIT 1;");
  $query->bindValue(1, $wantedtime);

  $query->execute();
  $bangerid = $query->fetch();

  $query = $pdo->prepare("SELECT songid, AVG(rating), COUNT(rating) FROM ratings WHERE thetime >= ? GROUP BY songid HAVING COUNT(rating) > 2 ORDER BY 2 ASC LIMIT 1;");
  $query->bindValue(1, $wantedtime);
    ...
    <div class="Halfpic">
      ...
      <div class="Halftext">
        <div class="font-effect-anaglyph">
          ...
          <h5 style="margin-top:50px; margin-bottom: 10px;">THE NIGHTMARE, EXPRESSED AS SCORES</h5>
        </div>
        <div>
          ...
          <?php } ?>
    	    <p class="intro">Let's not kid ourselves. There are no bangers tonight. Only complete shit.</p>
          <p class="intro">The least shit song tonight is...</p>
        </div>
        <p>
        ...
        <?php
        $top = new Raati;
        $top10 = $top->martinratedwith10($wantedtime);
        if (empty($top10)) { ?>
