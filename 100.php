...
if (isset($_SESSION['logged_in_levy'])) {
  ...
  $currenttime = time();
  $wantedtime = 1654358400;
  $endtime = 1654358400 + 86400;

  $query = $pdo->prepare("SELECT songid, AVG(rating), COUNT(rating) FROM ratings WHERE thetime >= ? AND thetime <= ? GROUP BY songid HAVING COUNT(rating) > 3 ORDER BY 2 DESC LIMIT 1;");
  ...
  $query = $pdo->prepare("SELECT songid, AVG(rating), COUNT(rating) FROM ratings WHERE thetime >= ? AND thetime <= ? GROUP BY songid HAVING COUNT(rating) > 3 ORDER BY 2 ASC LIMIT 1;");
  $query->bindValue(1, $wantedtime);
  $query->bindValue(2, $endtime);
  ...
  $query = $pdo->prepare("SELECT songid, aavg, @curRank := @curRank + 1 AS rank FROM (SELECT songid, AVG(rating) AS aavg, COUNT(rating) FROM ratings , (SELECT @curRank := 0) r GROUP BY songid HAVING COUNT(rating) > 3 ORDER BY aavg DESC) dr GROUP BY songid ORDER BY  rank ASC");
    ...
    <div class="Halfpic">
      ...
      <div class="Halftext">
        <div class="font-effect-anaglyph">
          ...
          <h5 style="margin-top:50px; margin-bottom: 10px;">LEVRAATI 100 MEMORIAL</h5>
          <h5 style="margin-top:50px; margin-bottom: 10px; color: green;">WHILE JANNE WAS HOSTAGE IN THE NIGHTMARE</h5>
        </div>
        <div>
          ...
          <p class="intro">Although this night was an audio nightmare, it was the first night Janne graced the Levyraati with his presence.</p>
          <p class="intro">The least shit song of Levyraati 100 when Janne was around was...</p>
        </div>
        <p>
        ...
        <a href="100nojanne.php">
        <div class="commentbutton" style="margin-top:50px; margin-bottom: 10px;">
        <p style="text-align:center;"><br>&#9201; AFTER JANNE PASSED OUT &#9201;<br><br></p>
        </div>
        </a>
        <p></p>
        ...
        <?php
        /* $top = new Raati; */
        /* $top10 = $top->janneratedwith10($wantedtime); */


          $query = $pdo->prepare("SELECT songby, songid, AVG(rating), COUNT(rating) FROM ratings WHERE thetime >= ? AND thetime <= ? GROUP BY songid HAVING COUNT(rating) > 3 ORDER BY 3 ASC LIMIT 10;");
          $query->bindValue(1, $wantedtime);
          $query->bindValue(2, $endtime);
