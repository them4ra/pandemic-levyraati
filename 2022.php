...
if (isset($_SESSION['logged_in_levy'])) {
  ...
  $currenttime = time();
  $wantedtime = (1641074400);

  $query = $pdo->prepare("SELECT songid, AVG(rating) FROM ratings WHERE thetime BETWEEN 1641074400 AND 1672264800 GROUP BY songid ORDER BY 2 DESC LIMIT 1;");
  $query->bindValue(1, $wantedtime);

  $query->execute();
  $bangerid = $query->fetch();

  $query = $pdo->prepare("SELECT songid, AVG(rating) FROM ratings WHERE thetime BETWEEN 1641074400 AND 1672264800 GROUP BY songid ORDER BY 2 ASC LIMIT 1;");
  $query->bindValue(1, $wantedtime);
    ...
    <div class="Halfpic">
      ...
      <div class="Halftext">
        ...
        <div>
          ...
          <p class="intro">What was the best song of 2022? LET'S FIND OUT...</p>
          <hr width="80%" color="#FFFFFF" size="2" align="left" margin-bottom="-5px">
            <div width="80%">
              <h3 width="80%">THE ABSOLUTE BEAST OF 2022....</h3>
            </div>
        </div>
        <p>
        ...
        <h3 width="80%">The SHAME song of 2022 was...</h3>
        <?php if (empty($getbanger)){ ?>
            <h5>No one put on any songs today!</h5>
        <?php } else {?>
          <div style="width: 80%;">
...
?>
        ...
        <?php
                ...
                $query = $pdo->prepare("SELECT songby, songid, rating, thetime FROM ratings WHERE thetime BETWEEN 1641074400 AND 1672264800 AND author= ? GROUP BY songid ORDER BY 3 DESC LIMIT 20;");
                ...
                <hr width="80%" color="#FFFFFF" size="2" align="left" margin-bottom="-5px">
                <h3 style="width: 80%;"><a href="user.php?id=<?php echo $id;?>"><b style="color: #00ff00;"><?php echo $id; ?></b></a>'s Favorite Songs of 2022 </h3>
                <div style="width:80%">
                  <ol>
                <?php
                $bottom = new Raati;
        ...
        foreach ($allusers as $user) { ?>
          ...
          $query = $pdo->prepare("SELECT songby, songid, AVG(rating) FROM ratings WHERE thetime BETWEEN 1641074400 AND 1672264800 AND songby = ? GROUP BY songid ORDER BY 3 DESC LIMIT 1;");
          ...
          $query = $pdo->prepare("SELECT songby, songid, AVG(rating) FROM ratings WHERE thetime BETWEEN 1641074400 AND 1672264800 AND songby = ? GROUP BY songid ORDER BY 3 ASC LIMIT 1;");
          $query->bindValue(1, $user['name']);
      ...
      <h2>&#128169;    &#128169;    &#128169;</h2>
      <h2 style="width: 80%; color: white;">THE SHIT OF 2022 (NOT IN A GOOD WAY)</h2>
      <h2>&#128169;    &#128169;    &#128169;</h2>
      <hr width="80%" color="#FFFFFF" size="2" align="left" margin-bottom="-5px">
