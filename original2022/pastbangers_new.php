...
if (isset($_SESSION['logged_in_levy'])) {
  ...
  */
  ...
  $query = $pdo->prepare("SELECT songid, COUNT(rating), AVG(rating) FROM ratings WHERE thetime <= ? AND thetime > ? GROUP BY songid HAVING COUNT(rating) > 2 ORDER BY 3 DESC LIMIT 1;");
  ...
  $query = $pdo->prepare("SELECT songid, COUNT(rating), AVG(rating) FROM ratings WHERE thetime <= ? AND thetime > ? GROUP BY songid HAVING COUNT(rating) > 2 ORDER BY 3 ASC LIMIT 1;");
  $query->bindValue(1, $wantedtime);
  $query->bindValue(2, $lastweek);
  ...
  <body>
    ...
    <div class="Halfpic">
      ...
      <div class="Halftext">
        ...
        <a href="pastbangers_new.php?timestamp=<?php echo $lastweek?>">
        ...
        foreach ($allusers as $user) { ?>
          ...
          $query = $pdo->prepare("SELECT songby, songid, AVG(rating), COUNT(rating) FROM ratings WHERE songby= ? and thetime <= ? AND thetime > ? GROUP BY songid HAVING COUNT(rating) > 2 ORDER BY 3 DESC LIMIT 1;");
          $query->bindValue(1, $user['name']);
          $query->bindValue(2, $wantedtime);
          $query->bindValue(3, $lastweek);
          ...
          $query = $pdo->prepare("SELECT songby, songid, AVG(rating), COUNT(rating) FROM ratings WHERE songby= ? and thetime <= ? AND thetime > ? GROUP BY songid HAVING COUNT(rating) > 2 ORDER BY 3 ASC LIMIT 1;");
          $query->bindValue(1, $user['name']);
          $query->bindValue(2, $wantedtime);
          $query->bindValue(3, $lastweek);
        ...
        <?php
        $top = new Raati;
        $top10 = $top->past_top10_afterhour($wantedtime, $lastweek);
        if (empty($top10)) { ?>
